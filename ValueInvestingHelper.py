...
class ValueInvestingHelper(DataBaseHelper):
    ...
    def processData(self, marketE):
        with open('analysis_data/'+marketE.name+'_balanceSheet.txt', 'r') as input:
            ...
            try:
                if keyB == keyI:
                    ...
                    if yearsI == yearsB:
                        ...
                        self.roe_dict[keyB] = (round(sum(roe)/len(roe),2), roe)
                        ...
                        try:
                            ...
                            self.epsg_dict[keyB] = (round(sum(epsg)/len(epsg),2), epsg)
                        except:
                            count_eps_0 += 1
                            self.epsg_dict[keyB] = (0, eps)
            except:
                count_missing += 1
