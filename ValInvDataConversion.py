...
class ValInvDataConversion(DataBaseHelper):
    ...
    def Logger(self, file_name):
        ...
        logging.basicConfig(filename = '%s.log' %(file_name),format= '%(asctime)s %(module)s,line: %(lineno)d %(levelname)8s | %(message)s',
                                        datefmt='%Y/%m/%d %H:%M:%S', filemode = 'a', level = logging.DEBUG)
    ...
    def cashStatementConversion(self, marketE):
        ...
        anal_file = '/home/colin/development/python/dash-trading/analysis_data/'+marketE.name+'_cashStatement.txt'
        ...
        for ticker in cashStatement:
            ...
            for bs in cashStatement[ticker]:
                ...
                try:
                    df.to_sql("val_inv_anal_cashsheet", con=DataBaseHelper.engine, if_exists='append',index=False) 
    ...
    def incomeStatementConversion(self, marketE):
        ...
        anal_file = '/home/colin/development/python/dash-trading/analysis_data/'+marketE.name+'_incomeStatement.txt'
        info=open(anal_file,'r')
        ...
        for ticker in incomeStatement:
            ...
            for bs in incomeStatement[ticker]:
                ...
                df['market']=marketE.name
                try:
                    df.to_sql("val_inv_anal_incomesheet", con=DataBaseHelper.engine, if_exists='append',index=False) 
                    DataBaseHelper.session.commit()
                except Exception as e:
                    logging.getLogger().exception(str(e))
                    logging.getLogger().debug(ticker+" insert failed")
    ...
    def balanceSheetConversion(self, marketE):
        ...
        anal_file = '/home/colin/development/python/dash-trading/analysis_data/'+marketE.name+'_balanceSheet.txt'
        info=open(anal_file,'r')
        balanceSheet = ast.literal_eval(info.read())
        dfTickers = pd.read_sql_query("SELECT * FROM public.val_inv_anal_balsheet;", con=DataBaseHelper.conn)
        ...
        for ticker in balanceSheet:
            ...
            for bs in balanceSheet[ticker]:
                ...
                df['market']=marketE.name
                try:
                    df.to_sql("val_inv_anal_balsheet", con=DataBaseHelper.engine, if_exists='append',index=False) 
                    DataBaseHelper.session.commit()
                except Exception as e:
    ...
    def clearTableData(self,marketE, tableName):
        ...
        for ticker in tickers:
            ...
            try:
                nm='"'+nm+'"'
                DataBaseHelper.conn.execute("DELETE FROM public."+tableName+"WHERE epic="+nm)
                DataBaseHelper.session.commit()
            except Exception as e:
                logging.getLogger().debug(str(e))
    ...
    def updateTableWithMarket(self, marketE):
        tickers = self.get_stocks_list(marketE)
        for tb in "val_inv_anal_balsheet", "val_inv_anal_cashsheet", "val_inv_anal_incomesheet":
            try:
                    dfTickers = pd.read_sql_query("SELECT * FROM public."+tb+";", con=DataBaseHelper.conn)
                    for ticker in tickers:
                        lTicker=ticker.ticker.lower()
                        lTicker="'"+lTicker+"'"
                        dfTickers['market'][dfTickers['epic'] == lTicker] = marketE.name
                    dfTickers.to_sql(tb, con=DataBaseHelper.engine, if_exists='replace',index=False) 
            except Exception as e:
                    print(str(e))
